...
const EXPORT_COLUMNS = [...LEAD_COLUMNS, { key: "metadata", label: "Metadata" }, { key: "id", label: "Lead ID" }];
...
function formatCell(row, column) {
  ...
  if (!value && ["metadata"].includes(column.key)) {
    return displayValue(readMetadata(row.metadata));
  }
...
export default function LeadSummary({ rows, sourceDatabase = "valuexpert" }) {
  ...
  async function copyLeadIds(leadIds) {
    ...
    try {
      const response = await fetch("/api/manager/lead", {
        method: "POST",
  ...
  function handleExport() {
    ...
    downloadCsv(
      ...
      exportRows,
      `${sourceDatabase}_Lead_${new Date().toISOString().slice(0, 10)}`
    );
  }
